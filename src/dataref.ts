...
import { type DataRef, DataRefType, DataRefTypesSet } from "./types";
...
export const setGlobalFetchBlobFromKey = (
...
export const dataRefToBuffer = async (
  ...
  switch (type) {
    ...
    case DataRefType.key: {
      ...
      if (!fetcher) {
        throw new Error(
          "No fetchBlobFromKey function provided, and setGlobalFetchBlobFromKey not called"
        );
      }
      const arrayBufferFromKey = await fetcher(ref.value, fetchOptions);
...
export const dataRefToFile = async (
  ...
  switch (type) {
    case DataRefType.base64:
      ...
      return new File([bufferBase64], name, {
    ...
    case DataRefType.url:
      const bufferUrl = await urlToUint8Array(
        ref.value as string,
        fetchOptions
      );
